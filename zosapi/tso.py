    ...
    def issue_tso_command(
        ...
        verify: bool = True
            ...
            try:
                ...
                self.log.critical(f'TSO-001S Catched an unexpected exception, can not continue {str(TSO.errors)}')
                sys.exit(TSO.rc)

            if response.status_code != 200:
                self.log.error(f"TSO-002E An unexpected statuscode {response.status_code} has been received")
                self.log.error(f"           {response.text}")
                TSO.rc = 8
                TSO.errors = {"rc": {TSO.rc}, "status_code": response.status_code, "reason": response.reason}
